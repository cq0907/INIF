...
template <typename scalar_t>
__global__ void sample_pts_on_rays_cuda_kernel1(
        scalar_t* __restrict__ rays_start,
        scalar_t* __restrict__ rays_d,
        scalar_t* __restrict__ xyz_min,
        scalar_t* __restrict__ xyz_max,
        int64_t* __restrict__ ray_id,
        int64_t* __restrict__ step_id,
        scalar_t* __restrict__ stepdist,
        const int total_len,
        scalar_t* __restrict__ rays_pts,
        bool* __restrict__ mask_outbbox) {
  const int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if(idx<total_len) {
    const int i_ray = ray_id[idx];
    const int i_step = step_id[idx];
    const float distance = stepdist[idx];

    const int offset_p = idx * 3;
    const int offset_r = i_ray * 3;
    const float dist = distance * i_step;
    const float px = rays_start[offset_r  ] + rays_d[offset_r  ] * dist;
    const float py = rays_start[offset_r+1] + rays_d[offset_r+1] * dist;
    const float pz = rays_start[offset_r+2] + rays_d[offset_r+2] * dist;
    rays_pts[offset_p  ] = px;
    rays_pts[offset_p+1] = py;
    rays_pts[offset_p+2] = pz;
    mask_outbbox[idx] = (xyz_min[0]>px) | (xyz_min[1]>py) | (xyz_min[2]>pz) | \
...
std::vector<torch::Tensor> sample_pts_on_rays_cuda1(
        torch::Tensor in_pts, torch::Tensor out_pts,
        torch::Tensor N_steps, torch::Tensor rays_d,
        torch::Tensor stepdist, torch::Tensor xyz_min,
        torch::Tensor xyz_max) {
  const int threads = 256;
  const int n_rays = in_pts.size(0);
  ...
  auto rays_pts = torch::empty({total_len, 3}, torch::dtype(in_pts.dtype()).device(torch::kCUDA));
  auto mask_outbbox = torch::empty({total_len}, torch::dtype(torch::kBool).device(torch::kCUDA));

  auto rays_start = in_pts;

  AT_DISPATCH_FLOATING_TYPES(in_pts.type(), "sample_pts_on_rays_cuda1", ([&] {
    sample_pts_on_rays_cuda_kernel1<scalar_t><<<(total_len+threads-1)/threads, threads>>>(
        rays_start.data<scalar_t>(),
        rays_d.data<scalar_t>(),
        xyz_min.data<scalar_t>(),
        xyz_max.data<scalar_t>(),
        ray_id.data<int64_t>(),
        step_id.data<int64_t>(),
        stepdist.data<scalar_t>(),
        total_len,
        rays_pts.data<scalar_t>(),
        mask_outbbox.data<bool>());
  }));
  return {rays_pts, mask_outbbox, ray_id, step_id};
}
...
__global__ void raw2alpha_backward_cuda_kernel(
  ...
  if(i_pt<n_pts) {
    ...
    grad[i_pt] = (min(exp_d[i_pt], 1e10) / (1 + exp_d[i_pt])) * grad_back[i_pt];
  }
}
