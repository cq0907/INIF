...
std::vector<torch::Tensor> sample_pts_on_rays1(
        torch::Tensor in_pts, torch::Tensor out_pts,
        torch::Tensor N_steps, torch::Tensor rays_d,
        torch::Tensor stepdist, torch::Tensor xyz_min,
        torch::Tensor xyz_max) {
  ...
  CHECK_INPUT(N_steps);
  CHECK_INPUT(rays_d);
  CHECK_INPUT(stepdist);
  CHECK_INPUT(xyz_min);
  CHECK_INPUT(xyz_max);
  return sample_pts_on_rays_cuda1(in_pts, out_pts, N_steps, rays_d, stepdist, xyz_min, xyz_max);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("sample_pts_on_rays1", &sample_pts_on_rays1, "Sample points on rays 1");
