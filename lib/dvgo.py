...
class DirectVoxGO(torch.nn.Module):
    def __init__(self, xyz_min, xyz_max,
        ...
        self._set_grid_resolution2D(num_voxels)

        # init density voxel grid
        self.density_type = density_type
        self.density_config = density_config
        self.density = grid.create_grid(
                density_type, channels=3, world_size=self.world_size,
                xyz_min=self.xyz_min, xyz_max=self.xyz_max,
                config=self.density_config)

        self.embed_fn, self.input_ch = get_embedder(multires, i_embed)
        self.skips = skips
        self.mlps = NeRF(D=mlp_depth, W=mlp_width,
                         input_ch=self.input_ch+3, output_ch=1, skips=skips,
    ...
    def _set_grid_resolution2D(self, num_voxels):
        # Determine grid resolution
        self.num_voxels = num_voxels
        self.voxel_size = ((self.xyz_max[:2] - self.xyz_min[:2]).prod() / num_voxels).pow(1/2)
        self.world_size = ((self.xyz_max[:2] - self.xyz_min[:2]) / self.voxel_size).long()
        self.world_size = torch.cat([self.world_size, torch.ones((1,))], -1).to(torch.int)
        self.voxel_size_ratio = 1.0
        print('dvgo: voxel_size      ', self.voxel_size)
        print('dvgo: world_size      ', self.world_size)
        print('dvgo: voxel_size_base ', self.voxel_size_base)
        print('dvgo: voxel_size_ratio', self.voxel_size_ratio)
    ...
    def sample_ray1(self, rays_o, rays_d, near, far, stepsize, **render_kwargs):
    ...
    def sample_ray(self, in_pts, out_pts, **render_kwargs):
        """

        """
        in_pts = in_pts.contiguous()
        out_pts = out_pts.contiguous()
        dir_pts = out_pts - in_pts
        rays_d = dir_pts / dir_pts.norm(dim=-1, keepdim=True)
        rays_d = rays_d.contiguous()
        stepdist = dir_pts.norm(dim=-1, keepdim=True) / self.N_sample
        stepdist = stepdist.repeat(1, self.N_sample+1)
        stepdist = torch.reshape(stepdist, (-1,))
        N_steps = torch.tensor([self.N_sample+1]*len(in_pts))

        N_steps = N_steps.contiguous()
        local_distance = stepdist.contiguous()
        ray_pts, mask_outbbox, ray_id, step_id = render_utils_cuda.sample_pts_on_rays1(
            in_pts, out_pts, N_steps, rays_d, stepdist, self.xyz_min, self.xyz_max)

        # mask_inbbox = ~mask_outbbox
        # ray_pts = ray_pts[mask_inbbox]
        # ray_id = ray_id[mask_inbbox]
        # step_id = step_id[mask_inbbox]
        return ray_pts, ray_id, step_id, local_distance
    ...
    def forward(self, xyz):
        ...
        '''
        xyz_flat = torch.reshape(xyz, [-1, xyz.shape[-1]])

        density = self.density(xyz_flat)
        raw = self.activate_density(density, None)
        # raw = raw.unsqueeze(-1)

        embedded = self.embed_fn(xyz_flat)
        embedded = torch.cat([raw, embedded], -1)

        sigma = self.mlps(embedded)
        return sigma
...
class Raw2Alpha(torch.autograd.Function):
    ...
    def forward(ctx, density, shift, interval):
        ...
        alpha = softplus(density + shift)
              = log(1 + exp(density + shift))
        '''
    ...
    def backward(ctx, grad_back):
        ...
        alpha' = 1 / (1 + exp(density + shift)) * exp(density + shift)'
               = 1 / (1 + exp(density + shift)) * exp(density + shift)
        '''
