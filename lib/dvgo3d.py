...
class DirectVoxGO(torch.nn.Module):
    def __init__(self, xyz_min, xyz_max,
        ...
        self._set_grid_resolution3D(num_voxels)
    ...
    def _set_grid_resolution3D(self, num_voxels):
    ...
    def sample_ray(self, in_pts, out_pts, N_sample, **render_kwargs):
        """
        ...
        rays_d = rays_d.contiguous()
        stepdist = dir_pts.norm(dim=-1, keepdim=True) / N_sample
        stepdist = stepdist.repeat(1, N_sample+1)
        stepdist = torch.reshape(stepdist, (-1,))
        N_steps = torch.tensor([N_sample+1]*len(in_pts))
    ...
    def forward(self, in_pts, out_pts, N_sample):
        '''Volume rendering
        ...
        ray_pts, ray_id, _, local_distance = self.sample_ray(in_pts, out_pts, N_sample)
        density = self.density(ray_pts)
        raw = self.activate_density(density, None)
        # raw = raw.unsqueeze(-1)

        embedded = self.embed_fn(ray_pts)
        embedded = torch.cat([raw, embedded], -1)
        sigma = self.mlps(embedded)

        rgb_marched = segment_coo(src=sigma, index=ray_id, out=torch.zeros([N, 1]),reduce='sum')
        ret_dict.update({
